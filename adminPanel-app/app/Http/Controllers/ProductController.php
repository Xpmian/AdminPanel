...
class ProductController extends Controller
{
    ...
    {
        $kategoriler = Category::where('status',1)->orderBy('productTitle', 'asc')
                                                  ->get();
    ...
    public function edit_urun(Request $request ,$id)
    {
        ...
        $existingUser = Product::where('productTitle', $productTitle)
                       ->where('id', '<>', $id)
                       ->first();
        ...
        {
            return redirect()->back()->withErrors(['error' => 'Bu isme sahip bir ürün var!']);
        }
        else
        {
            if($request->filled('ProductTitle') && $request->filled('Barcode') && $request->filled('price') && $request->filled('stock'))
            {
                ...
                $urun->productStatus = $urunStatus;
                $urun->price = $request->input('price');
                $urun->stock = $request->input('stock');
                $urun->slug = $this->createSlug($request->input('ProductTitle'));
                $urun->save();
